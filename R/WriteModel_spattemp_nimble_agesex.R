...
code <- nimbleCode({
    ...
    for(r in 1:R){
      for(c in 1:C){
          ...
          for(s in 1:S){
            ...
            pep_ctras[c,t,r,1:A,s] ~ dmulti(prob_ctras[c,t,r,1:A,s], pep_pop_ctrs[c,t,r,s])
            prob_ctras[c,t,r,1:A,s] ~ ddirch(Alpha[1:A])
            }
          }
      ...
      for(t in 1:nyears){
        ...
        for(a in 1:A){
          for(s in 1:S){
            gamma_ctras[c,t,r,a,s] <- gamma_ctr[c,t,r] * prob_ctras[c,t,r,a,s]
          }
        }
      }
  ...
  for(c in 1:C){
    ...
    xi_ctr[c,1,hisp_index] ~ T(dnorm(hisp_undercount_2010, sd = hisp_sd_2010), ,0)
    ...
    xi_ctr[c,2,hisp_index] ~ T(dnorm(hisp_undercount_2020, sd  = hisp_sd_2020), 0,)
